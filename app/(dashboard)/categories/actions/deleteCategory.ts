...
if(!session) return throwCustomError('Unauthorized')
    if(!id) return throwCustomError('id is required')
        ...
        const relatedSubCategories = await prisma.subCategory.count({
            where: {
              categoryId: id,
            },
          });
          ...
          return throwCustomError(`Unable To Delete - You have ${relatedSubCategories} blog${relatedSubCategories > 1 ? 's' : ''} Related To This Category`)
          }
